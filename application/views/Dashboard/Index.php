...
/* Default progress bar style with gradient */
.progress-bar {
  height: 100%;
  border-radius: 30px;
  background-image: linear-gradient(to right, rgba(255,255,255,0.15) 0%, rgba(255,255,255,0.3) 100%);
...
/* Progress bar color variants with gradients */
.progress-bar-red {
  background-color: #e74c3c;
  background-image: linear-gradient(45deg, #e74c3c 25%, #c0392b 25%, #c0392b 50%, #e74c3c 50%, #e74c3c 75%, #c0392b 75%, #c0392b 100%);
}

.progress-bar-yellow {
  background-color: #f39c12;
  background-image: linear-gradient(45deg, #f39c12 25%, #e67e22 25%, #e67e22 50%, #f39c12 50%, #f39c12 75%, #e67e22 75%, #e67e22 100%);
}

.progress-bar-green {
  background-color: #2ecc71;
  background-image: linear-gradient(45deg, #2ecc71 25%, #27ae60 25%, #27ae60 50%, #2ecc71 50%, #2ecc71 75%, #27ae60 75%, #27ae60 100%);
}
...
/* Animated count-up for progress number */
.count-up {
...
/* Additional JS for functionality - Add this to your scripts */
</style>
<script>
// Add data-tooltip attributes to progress bars with percentage
document.addEventListener('DOMContentLoaded', function() {
  // Set tooltip data for progress bars
  const progressBars = document.querySelectorAll('.progress');
  progressBars.forEach(function(progressBar) {
    const bar = progressBar.querySelector('.progress-bar');
    if (bar) {
      const width = bar.style.width;
      progressBar.setAttribute('data-tooltip', 'Progres: ' + width);
    }
  });
  
  // Add count-up animation to progress numbers
  const progressNumbers = document.querySelectorAll('.progress-number');
  progressNumbers.forEach(function(element) {
    element.classList.add('count-up');
  });
  
  // Add smooth animation when progress bars come into view
